...
// Umbral competitivo
const ELO_MINIMO: f64 = 1000.0;
const TIEMPO_SNAPSHOT_SEGUNDOS: i32 = 1200; // Minuto 20

struct PartidaFiltrada {
    match_id: String,
    avg_elo: f64,
    winner_target: f64, // 1.0 si es p1, 0.0 si es p2
...
pub fn ejecutar_limpieza() -> Result<(), Box<dyn Error>> {
    ...
    // 1. RUTAS ADAPTADAS A TU ESTRUCTURA ACTUAL
    // Asumimos que sample_snapshots_t_1200.csv está directamente en la raíz de data/ o data/raw/
    // Si está dentro de data/raw/, cambia la ruta a "../data/raw/sample_snapshots_t_1200.csv"
    ...
    // Lectura dinámica de encabezados del Snapshot
    ...
    println!("Filtrando partidas desde el maestro (Arabia con ELO >= {})...", ELO_MINIMO);
    for result in rdr.records() {
        ...
        if elo >= ELO_MINIMO {
            ...
            let winner_str = record.get(idx_winner).unwrap_or("");
            
            // Asignación binaria del Ganador
            let winner_target = if winner_str == "p1" { 1.0 } else { 0.0 };
    ...
    println!("Se identificaron {} partidas potenciales.", partidas_candidatas.len());

    // 2. EXTRACCIÓN MÚLTI-HILO DE LOS LOGS DE INPUTS
    ...
    let registros_limpios: Vec<FilaLimpia> = partidas_candidatas.par_iter().filter_map(|partida| {
        ...
        if !Path::new(&ruta_input_match).exists() {
            return None; // Salto seguro si el archivo de inputs de esa partida no se descargó
        }
        ...
        // Lectura dinámica de encabezados del archivo de Inputs (Previene desfases por índices vacíos)
        ...
        for row_res in input_rdr.records() {
            if let Ok(row) = row_res {
                ...
                // Restricción temporal estricta (Minuto 20)
                ...
                // Aquí Rust hace el "match" directo usando el valor de la columna 'player'
                if jugador == "p1" {
                    p1_act += 1.0;
                    match tipo_accion {
                        "Queue" => p1_queues += 1.0,
    ...
    // 3. ESCRITURA DEL DATASET CENTRALIZADO
